...
uint64_t mbw_test_monitor_uuid_hi(uint32_t display_id) {
  if (display_id == 0) {
    return 0;
  }
  ...
  uint64_t hi = 0;
  for (int i = 0; i < 8; i++) {
    hi = (hi << 8) | (uint64_t)raw[i];
  }
  return hi;
}

uint64_t mbw_test_monitor_uuid_lo(uint32_t display_id) {
  if (display_id == 0) {
    return 0;
  }
  ...
  uint64_t lo = 0;
  for (int i = 8; i < 16; i++) {
    lo = (lo << 8) | (uint64_t)raw[i];
  }
  return lo;
}

uint32_t mbw_test_monitor_invalid_uuid_display_id(void) {
  CFUUIDRef uuid = CFUUIDCreate(NULL);
  if (uuid == NULL) {
    return 0;
  }
  uint32_t display_id = (uint32_t)CGDisplayGetDisplayIDFromUUID(uuid);
  CFRelease(uuid);
  return display_id;
}
