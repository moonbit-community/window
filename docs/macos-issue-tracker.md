...
| MBW-MAC-029 | GitHub issue #5 / native callback bridge | Native callback trampolines invoked stored MoonBit closures without holding an invocation-local reference, so callback-driven cleanup/reinstall could release a closure while native code was still invoking it. | DONE | `macos/native_appkit.m` now `moonbit_incref`/`moonbit_decref`s each stored callback closure around window/input/text/device/sync/lifecycle trampoline invocation; `examples/pump_events` executable ran for 8s without invalid memory access |
