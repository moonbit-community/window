...
- Completed: aligned fullscreen window-creation positioning with upstream: initial `attributes.position` is now ignored whenever `fullscreen` is set, so fullscreen-target placement is no longer overwritten by a later generic position apply step.
