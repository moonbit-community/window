...
- Completed: fullscreen transition and queued target state are now owned together by MoonBit per-window fullscreen state, matching upstream `in_fullscreen_transition` / `target_fullscreen` ownership instead of storing delayed requests in global `app_state`.
- Completed: fullscreen transition processing remains triggered by `app_state_queue_native_window_event` callback kinds `11/12/13`, but callback handling now consumes the target through the per-window fullscreen seam.
...
- Completed: `pump_app_events` control flow has been simplified to launch/rerun/timeout branches around direct `NSApplication::run`. The MoonBit-specific `try_pump_app_events` variant can return `EventLoopError` when runtime main-thread validation fails; upstream enforces the same affinity statically because its AppKit event loop is not `Send`.
