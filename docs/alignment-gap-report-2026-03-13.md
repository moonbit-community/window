...
- Completed: migrated IME property/cursor-area bridges (`ime_allowed`, `set_ime_cursor_area`, `ime_cursor_{x,y,width,height}`) from dedicated C exports into MoonBit-composed ObjC primitive calls (`imeAllowed`, `setImeCursor*`, `inputContext.invalidateCharacterCoordinates`), while keeping `set_ime_allowed` in C for its state-machine side effects.
...
- Completed: added low-level ObjC primitive signatures required by the above migrations (`mbw_objc_msg_send_void_rect_bool` and `mbw_objc_msg_send_void_i32_double_double_i32_u64_i32_i32_u64`).
...
- Completed: text-input callback ABI now carries raw `NSString` handles (`text_handle`/`path_handle`) plus minimal numeric context, and MoonBit `view.mbt` performs UTF-8 decoding (`objc_utf8_bytes_from_object`) before event construction.
